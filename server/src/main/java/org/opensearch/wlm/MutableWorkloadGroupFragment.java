...
public class MutableWorkloadGroupFragment extends AbstractDiffable<MutableWorkloadGroupFragment> {
    ...
    private final Map<String, Function<XContentBuilder, Void>> toXContentMap = Map.of(RESILIENCY_MODE_STRING, (builder) -> {
        ...
        try {
            ...
            if (q.isEmpty() == false) {
                ...
                if (q.hasValue(WorkloadGroupQueueSettings.SIZE.getKey())) {
                    builder.field(WorkloadGroupQueueSettings.SIZE.getKey(), WorkloadGroupQueueSettings.SIZE.get(q));
                }
                if (q.hasValue(WorkloadGroupQueueSettings.TIMEOUT.getKey())) {
                    builder.field(WorkloadGroupQueueSettings.TIMEOUT.getKey(), WorkloadGroupQueueSettings.TIMEOUT.get(q).getStringRep());
                }
                builder.endObject();
            }
            return null;
    ...
    public void writeTo(StreamOutput out) throws IOException {
        ...
        if (out.getVersion().onOrAfter(Version.V_3_7_0)) {
            ...
            // TODO: when upstreamed, gate throttling/queue behind their own release version to support mixed-build clusters.
